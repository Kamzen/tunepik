...
class Http {
  ...
  getNetMessage (text) {
    return `<center>${Icons.noNetwork({
      color: '',
      width: 30,
      height: 30
    })}<br /><span class="app-max-text">${text}</span></center>`
  }
  ...
  request (callback, args) {
    ...
    try {
      ...
      self.xhr.setRequestHeader(
        'X-CSRF-TOKEN',
        $.get('.app-token').getAttribute('content')
      )
      ...
      xhrError.message =
        'Encountered An Error Displaying This Page : ' + e.toString()
...
class Element {
  ...
  constructor (dom) {
    ...
    this.isMobile = !(screen.width > 699)
  } // End Of Constructor
...
class UserAnalytics extends UserModel {
  ...
  getMediaInteraction () {
    return this.media_interaction
  } // End Of Media Interaction
...
class PostModel extends UserModel {
  ...
  constructor (args) {
    ...
    this.shared_post = {
      ...
      model: args.is_original_post
        ? ''
        : args.original_post
          ? new PostModel(args.original_post)
          : ''
    }
...
class NotificationModel extends UserModel {
  constructor (args) {
    ...
    this.extras = {
      ...
      sharedPost: args.notif_share_post
        ? new globs.model.post(args.notif_share_post)
        : null,
...
const globs = {
  ...
  trim: function (string, limit) {
    ...
    return string.toString().length > limit
      ? `${string.toString().substring(0, limit)}...`
      : string.toString()
  },
  reg: function (text) {
